...
const TelaPostagens = () => {
  ...
  const criarPostagem = async () => {
    ...
    AsyncStorage.getItem('token')
      .then((token) => {
        return api.post('/CriarPostagem', { Descricao: descricaoLimpa }, {
          headers: { authorization: token },
        });
      })
      .then(async (response) => {
  ...
  const reagirPostagem = (postID, tipo) => {
    AsyncStorage.getItem('token')
      .then((token) => {
        return api.post('/ReagirPostagem', { postID, tipo }, {
          headers: { authorization: token },
        });
      })
      .then(() => {
  ...
  const comentarPostagem = (postID) => {
    ...
    AsyncStorage.getItem('token')
      .then((token) => {
        return api.post('/comentarPost', { postID, comentario }, {
          headers: { authorization: token },
        });
      })
      .then(() => {
...
const styles = StyleSheet.create({
  ...
  postContainer: { padding: 10, borderBottomWidth: 1, borderBottomColor: '#ddd' },
  userInfoContainer: { flexDirection: 'row', alignItems: 'center', marginBottom: 5 },
  ...
  input: { backgroundColor: '#fff', padding: 10, borderRadius: 8, width: '80%', marginBottom: 10 },
  button: { backgroundColor: '#007bff', padding: 10, borderRadius: 8, width: '80%', alignItems: 'center', marginBottom: 10 },
  buttonText: { color: '#fff' },
  selectedImage: { width: 100, height: 100, marginTop: 10, borderRadius: 8 },
  loadingText: { textAlign: 'center', marginTop: 20 },
  userResultContainer: { flexDirection: 'row', alignItems: 'center', padding: 10 },
  searchResults: { backgroundColor: '#fff', paddingHorizontal: 10, paddingTop: 5 },
});
